    ...
    public function copyQuotation(Request $request,$qasalId,$qtujuanId){
        ...
        try {
            ...
            foreach ($detail as $key => $value) {
                ...
                DB::table("sl_quotation_chemical")->where('quotation_id',$qtujuanId)->update([
                    "deleted_at" => $current_date_time ,
                    "deleted_by" => Auth::user()->full_name,
                ]);
                ...
                DB::table("sl_quotation_detail_requirement")->where('quotation_id',$qtujuanId)->update([
                    "deleted_at" => $current_date_time ,
                    "deleted_by" => Auth::user()->full_name,
                ]);
                ...
                DB::table("sl_quotation_detail_tunjangan")->where('quotation_id',$qtujuanId)->update([
                    "deleted_at" => $current_date_time ,
                    "deleted_by" => Auth::user()->full_name,
                ]);
                ...
                DB::table("sl_quotation_kaporlap")->where('quotation_id',$qtujuanId)->update([
                    "deleted_at" => $current_date_time ,
                    "deleted_by" => Auth::user()->full_name,
                ]);
    ...
    public function view (Request $request,$id){
        try {
            ...
            foreach ($quotation->quotation_detail as $ikbd => $kbd) {
                ...
                $kbd->tunjangan_holiday = 0;
                if($quotation->tunjangan_holiday != "Tidak Ada" && $quotation->tunjangan_holiday != "" && $quotation->tunjangan_holiday != null) {
                    if($quotation->tunjangan_holiday=="Flat"){
                        $kbd->tunjangan_holiday = $quotation->nominal_tunjangan_holiday;
                    }else{
                ...
                // dd($kbd->personil_kaporlap);
                $kbd->total_personil = $quotation->nominal_upah+$totalTunjangan+$kbd->tunjangan_hari_raya+$kbd->kompensasi+$kbd->tunjangan_holiday+$kbd->lembur+$kbd->nominal_takaful+$kbd->bpjs_jkk+$kbd->bpjs_jkm+$kbd->bpjs_jht+$kbd->bpjs_jp+$kbd->bpjs_kes+$kbd->personil_kaporlap+$kbd->personil_devices+$kbd->personil_chemical+$kbd->personil_ohc;
            ...
            return view('sales.quotation.view',compact('canCreateSpk','quotationTujuan','quotation','salaryRuleQ','quotationDetail','listPic','daftarTunjangan','listChemical','listDevices','listOhc','listKaporlap','listJenisChemical','listJenisDevices','listJenisOhc','listJenisKaporlap','now','data','master','leads','aplikasiPendukung'));
        } catch (\Exception $e) {
            dd($e);
            SystemController::saveError($e,Auth::user(),$request);
            abort(500);
    ...
    public function cetakQuotation (Request $request,$id){
        try {
            ...
            foreach ($data->quotation_detail as $ikbd => $kbd) {
                ...
                $personilChemical = 0;
                $kbdChemical = DB::table('sl_quotation_chemical')->whereNull('deleted_at')->where('quotation_id',$data->id)->get();
                foreach ($kbdChemical as $ikdbchemical => $kdbchemical) {
                    $personilChemical += ($kdbchemical->harga*$kdbchemical->jumlah)/$provisi;
                };
                ...
                // dd($kbd->personil_kaporlap);
                $kbd->total_personil = $data->nominal_upah+$totalTunjangan+$kbd->tunjangan_hari_raya+$kbd->kompensasi+$kbd->tunjangan_holiday+$kbd->lembur+$kbd->nominal_takaful+$kbd->bpjs_jkk+$kbd->bpjs_jkm+$kbd->bpjs_jht+$kbd->bpjs_jp+$kbd->bpjs_kes+$kbd->personil_kaporlap+$kbd->personil_devices+$kbd->personil_chemical;
                ...
                // Permintaan pak yaser Management Fee untuk COST STRUCTURE DITAMBAH TOTAL OHC 
                $kbd->management_fee_coss = ($kbd->sub_total_personil_coss*$data->persentase/100);
