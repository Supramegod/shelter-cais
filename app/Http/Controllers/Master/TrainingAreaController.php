...
class TrainingAreaController extends Controller
{
    public function index(Request $request){
        ...
        return view('master.training-area.list');
    }
    ...
    public function add(Request $request){
        ...
        return view('master.training-area.add',compact('now', 'listBu'));
    }

    public function view(Request $request,$id){
        try {
            $data = DB::table('m_training_area')->where('id',$id)->first();
            $listBu = DB::table('m_training_laman')->orderBy('laman', 'ASC')->get();

            return view('master.training-area.view',compact('data', 'listBu'));
        } catch (\Exception $e) {
            SystemController::saveError($e,Auth::user(),$request);
            abort(500);
        }
        ...
        try {
            ...
            if ($validator->fails()) {
                ...
                if(!empty($request->id)){
                    ...
                    DB::table('m_training_area')->insert([
                        'area'         => $request->area,
                        'laman_id'     => $request->laman_id,
                        'created_at'    => $current_date_time
                    ]);
                }
                $msgSave = 'Training area'.$request->nama.' berhasil disimpan.';
            }
            DB::commit();
