  ...
  <div class="container-fluid flex-grow-1 container-p-y">
    ...
    <div class="row">
            ...
            <div class="row mb-3">
              ...
              <div class="col-sm-9">
                <div class="form-check form-switch">
                  <input style="width: 60px; height: 30px;" class="form-check-input" type="checkbox" role="switch"
                    id="enable" name="enable" @if($data->enable == '1') checked @endif>
                </div>
              </div>
            </div>
              ...
              <div class="col-sm-3">
                <select disabled id="laman_id" name="laman_id" class="form-select">
                  @foreach($listBu as $value)
                    <option value="{{$value->id}}" @if($data->id_laman == $value->id) selected @endif>{{$value->laman}}
                    </option>
                  @endforeach
                </select>
              </div>
              ...
              <div class="col-sm-4">
                <select id="materi_id" name="materi_id" class="form-select @error('materi_id') is-invalid @enderror">
                  ...
                  @foreach($listMateri as $value)
                    <option value="{{$value->id}}" @if($data->id_materi == $value->id) selected @endif>{{$value->nama}}
                    </option>
                  @endforeach
                </select>
              </div>
      ...
      <div class="col-md-3">
        ...
        <!-- Card Gallery Training - Carousel dipindah ke sini -->
        <div class="card">
          <div class="card-header">
            <h5 class="card-title m-0">Gallery Training</h5>
          </div>
          <div class="card-body p-2">
            <div id="carouselExample" class="carousel slide">
              ...
              <div class="carousel-inner">
                @foreach($listImage as $value)
                  <div class="carousel-item @if($loop->index == 0) active @endif" data-id="{{ $loop->index }}"
                    data-path="{{ $value->path }}" data-nama="{{ $value->nama }}" style="height: 300px;">
                    <a href="#" class="view-image-link" data-bs-toggle="modal" data-bs-target="#imageModal">
                      <img style="border-radius: 1%; width: 100%; height: 100%; object-fit: cover;" src="{{ $value->path }}"
                        class="d-block w-100" alt="{{ $value->nama }}">
                    </a>
                    <div class="carousel-caption d-none d-md-block">
                      <div class="bg-dark bg-opacity-50 rounded p-2">
                        <h6 class="mb-1">{{$value->nama}}</h6>
                        <p class="mb-0 small">{{$value->keterangan}}</p>
                      </div>
                    </div>
                  </div>
                @endforeach
...
@section('pageScript')
  <script>
    document.addEventListener('DOMContentLoaded', function () {
      // Ambil semua data gambar dari carousel utama
      ...
      // Event listener untuk setiap link gambar
      document.querySelectorAll('.view-image-link').forEach(link => {
        link.addEventListener('click', function (event) {
          event.preventDefault();

          // Ambil index dari gambar yang diklik
          var clickedIndex = parseInt(this.closest('.carousel-item').getAttribute('data-id'));

          // Dapatkan carousel di dalam modal
          var modalCarouselInner = document.querySelector('#modalCarousel .carousel-inner');
          modalCarouselInner.innerHTML = ''; // Kosongkan konten sebelumnya

          // Buat dan tambahkan item carousel ke dalam modal
          imagesData.forEach(function (image, index) {
            var itemDiv = document.createElement('div');
            itemDiv.className = 'carousel-item';
            if (index === clickedIndex) {
              itemDiv.classList.add('active'); // Set gambar yang diklik sebagai aktif
            }
          ...
          // Inisialisasi carousel Bootstrap di dalam modal
          var modalCarousel = new bootstrap.Carousel(document.getElementById('modalCarousel'));
          modalCarousel.to(clickedIndex); // Pindahkan ke slide yang benar
        });
      });
    });
    // Fungsi untuk menampilkan gambar di modal dan membuka modal
    function showImageInModal(imagePath) {
      // 1. Mengubah sumber (src) gambar di dalam modal
      document.getElementById('modalImage').src = imagePath;

      // 2. Mendapatkan elemen modal
      var myModal = document.getElementById('imageModal');

      // 3. Membuat instance Modal Bootstrap
