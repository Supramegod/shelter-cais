                ...
                <div class="card mb-4">
                    <h5 class="card-header">
                        <div class="d-flex justify-content-between">
                            <span>Form PKS <span
                                    class="badge @if (
                                        $data->status_pks_id == 1 ||
                                            $data->status_pks_id == 2 ||
                                            $data->status_pks_id == 3 ||
                                            $data->status_pks_id == 4 ||
                                            $data->status_pks_id == 5) bg-label-warning @elseif($data->status_pks_id == 6) bg-label-info @elseif($data->status_pks_id == 7) bg-label-success @endif rounded-pill mt-1">{{ $data->status }}</span></span>
                    ...
                    <form class="card-body overflow-hidden" action="{{ route('leads.save') }}" method="POST">
                            ...
                            <div class="table-responsive">
                                <table class="table">
                                    <tbody>
                                        <tr>
                                            <td>Nomor</td>
                                            <td colspan="3">: <a href="#"><b>{{ $leads->nomor ?? '-' }}</b></a>
                                            </td>
                                        </tr>
                                        <tr>
                                    ...
                                    <tbody>
                                        @foreach ($listQuotation as $index => $quotation)
                                            <tr>
                                                <td>{{ $index + 1 }}</td>
                                                <td><b><a
                                                            href="{{ route('quotation.view', [$quotation->id]) }}">{{ $quotation->nomor }}</a></b>
                                                </td>
                                                <td>{{ $quotation->kebutuhan }}</td>
                                                <td>{{ $quotation->jenis_kontrak }}</td>
                                                <td>
                                                    <div class="d-flex justify-content-center gap-2 mt-2">
                                                        <a href="{{ route('pks.isi-checklist', ['id' => $quotation->id, 'pks_id' => $data->id]) }}"
                                                            class="btn btn-primary" title="Isi Checklist">
                                                            <i class="mdi mdi-pencil"></i>
                                                        </a>
                                                        @if ($quotation->materai != null)
                                                            <a onclick="window.open('{{ route('quotation.cetak-checklist', ['id' => $quotation->id, 'pks_id' => $data->id]) }}','name','width=600,height=400')"
                                                                rel="noopener noreferrer" href="javascript:void(0)"
                                                                class="btn btn-warning" title="Cetak Checklist">
                        ...
                        <div class="row mb-3">
                            <div class="table-responsive overflow-hidden table-spk">
                                <table id="table-spk" class="dt-column-search table w-100 table-hover"
                                    style="text-wrap: nowrap;">
                                    <thead>
                                        <tr>
                                            <th class="text-center">No.</th>
                                    ...
                                    <tbody>
                                        @foreach ($listSpk as $index => $spk)
                                            <tr>
                                                <td>{{ $index + 1 }}</td>
                                                <td><b><a
                                                            href="{{ route('spk.view', [$spk->id]) }}">{{ $spk->nomor }}</a></b>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                            ...
                            <div class="row mb-1">
                                <div class="table-responsive overflow-hidden table-quotation">
                                    <table id="table-quotation" class="dt-column-search table w-100 table-hover">
                                        <thead>
                                            <tr>
                                                <th style="text-align:center">{{ $value->pasal }} &nbsp; &nbsp; <a
                                                        href="{{ route('pks.edit-perjanjian', $value->id) }}"
                                                        class="btn btn-warning"><i class="mdi mdi-pencil"></i></a></th>
                                            </tr>
                                            <tr>
                ...
                <div class="row">
                    <div class="card h-100">
                        <div class="card-header d-flex align-items-center justify-content-between">
                            ...
                            <div class="dropdown">
                                <button class="btn p-0" type="button" id="upgradePlanCard" data-bs-toggle="dropdown"
                        ...
                        <div class="card-body">
                          ...
                          <a onclick="window.open('{{ route('pks.cetak-pks', $data->id) }}','name','width=600,height=400')" rel="noopener noreferrer" href="javascript:void(0)" id="btn-download-pks" class="btn btn-warning w-100 waves-effect waves-light">
                            ...
                            @if ($data->link_pks_disetujui != null)
                                <div class="col-12 text-center mt-2">
                                    <a target="_blank" href="{{ $data->link_pks_disetujui }}" id="btn-lihat-pks"
                            ...
                            @else
                                <div class="col-12 text-center mt-2">
                                    <a onclick="window.open('{{ route('pks.cetak-pks', $data->id) }}','name','width=600,height=400')"
                            ...
                            @if ($data->status_pks_id == 1 && Auth::user()->role_id == 96)
                                <div class="col-12 text-center mt-2">
                                    <button class="btn btn-primary w-100 waves-effect waves-light" id="approve-pks"
                                        data-id="{{ $data->id }}" data-ot="1"><i
                                            class="mdi mdi-draw-pen"></i>&nbsp; Approval Direktur Sales</button>
                                </div>
                            @elseif($data->status_pks_id == 2 && in_array(Auth::user()->role_id, [97, 40]))
                                <div class="col-12 text-center mt-2">
                                    <button class="btn btn-primary w-100 waves-effect waves-light" id="approve-pks"
                                        data-id="{{ $data->id }}" data-ot="2"><i
                                            class="mdi mdi-draw-pen"></i>&nbsp; Approval Direktur Keuangan</button>
                                </div>
                            @elseif($data->status_pks_id == 3 && Auth::user()->role_id == 53)
                                <div class="col-12 text-center mt-2">
                                    <button class="btn btn-primary w-100 waves-effect waves-light" id="approve-pks"
                                        data-id="{{ $data->id }}" data-ot="3"><i
                                            class="mdi mdi-draw-pen"></i>&nbsp; Approval GM HRM</button>
                                </div>
                        ...
                        <button class="btn btn-primary w-100 waves-effect waves-light" id="approve-pks" data-id="{{ $data->id }}" data-ot="4"><i class="mdi mdi-draw-pen"></i>&nbsp; Approval Direktur Utama</button>
                            ...
                            @elseif($data->status_pks_id == 6 && in_array(Auth::user()->role_id, [56, 2]))
                                <div class="col-12 text-center mt-2">
                                    <button class="btn btn-info w-100 waves-effect waves-light" id="aktifkan-site"
                                        data-id="{{ $data->id }}">
                            ...
                            <div class="col-12 text-center mt-2">
                                @if (!$data->isLowongan)
                                    <button class="btn btn-warning w-100 waves-effect waves-light" id="buat-lowongan"
                                        data-id="{{ $data->id }}">
                            ...
                            @if ($data->status_pks_id == 7 && in_array(Auth::user()->role_id, [56, 2]))
                                <div class="dropdown col-12 text-center mt-2">
                                    <button class="btn btn-warning dropdown-toggle w-100" type="button"
                                        data-bs-toggle="dropdown" aria-expanded="false">
                                        Buat Purchase Request
                                    </button>
                                    <ul class="dropdown-menu w-100">
                                      <li><a class="dropdown-item buat-pr-item"  data-jenis="Chemical">Chemical</a></li>
                                      <li><a class="dropdown-item buat-pr-item"  data-jenis="Kaporlap">Kaporlap</a></li>
                                      <li><a class="dropdown-item buat-pr-item" data-jenis="Devices">Devices</a></li>
                                    </ul>
                                </div>
...
@section('pageScript')
    <script>
                ...
                if (result.isConfirmed) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            let baseUrl =
                                "{{ route('pks.ajukan-ulang-quotation', ['pks' => ':pks']) }}";
                    ...
                    $.ajax({
                        ...
                        error: function(xhr, status, errorThrown) {
                            ...
                            Swal.fire({
                                ...
                                html: "Terjadi kesalahan saat membuat lowongan. Status: " +
                                    xhr.status,
                                icon: "error",
                            });
                        }
        ...
        $('body').on('click', '.buat-pr-item', function(e) {
    e.preventDefault();
    ...
    @if ($data->status_pks_id != 7)
        Swal.fire({
            icon: 'warning',
            title: 'Belum bisa membuat Purchase Request',
            text: 'Silakan aktifkan site terlebih dahulu sebelum membuat Purchase Request.',
            confirmButtonText: 'OK',
            customClass: {
                confirmButton: 'btn btn-primary waves-effect waves-light'
            },
    ...
    let url = "{{ url('purchase/purchase-request/save') }}"; // tanpa ID di URL
    ...
    let inputJenis = document.createElement('input');
    ...
    inputQuotationId.name = 'quotation_id';
    inputQuotationId.value = quotationId;
    form.appendChild(inputQuotationId);
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        error: function(xhr, status, errorThrown) {
                            ...
                            Swal.fire({
                                title: "Terjadi Kesalahan",
                                html: "Terjadi kesalahan saat memproses permintaan. Status: " +
                                    xhr.status,
                                icon: "error",
                            });
                        }
