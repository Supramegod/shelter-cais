@php
    $arrStep = [
        ...
        [ 'stepno' => 11 ,'step' => 'Harga Jual', 'info' => 'Informasi Harga Jual' ],
        [ 'stepno' => 12 ,'step' => 'Syarat dan Ketentuan', 'info' => 'Informasi Syarat dan Ketentuan' ]
    ];
@endphp
<div class="bs-stepper-header gap-lg-3 pt-5"  style="border-right:1px solid rgba(0, 0, 0, 0.1);">
    ...
    @foreach($arrStep as $key => $data)
        ...
        @if(!$loop->first)
            <div class="line"></div>
        @endif
        <a href="@if($data['stepno']<$quotation->step) {{route('quotation-sandbox.step',['id'=>$request->id,'step'=>$data['stepno']])}} @else javascript:void(0) @endif">
