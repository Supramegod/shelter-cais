...
<head>
    ...
    <style>
        ...
        /* Untuk menghindari page kosong di akhir */
...
<body>
    ...
    <div class="content">
        <div style="margin-top:50px;margin-right:20px;color:black !important;font-size:10pt !important">
            ...
            @foreach ($quotation->quotation_detail as $site)
                @php

                    $siteDetails = $quotation->quotation_detail->where('id', $site->id);
                    ...
                    <table class="bordered">
                        <thead class="text-center">
                            ...
                            <tr class="table-success">
                                ...
                                <th colspan="{{ 3 + count($quotation->quotation_detail) }}"
                                    style="vertical-align: middle;text-align:center">{{ $site->nama_site }}</th>
                                {{-- @endforeach --}}
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td class="fw-bold" style="text-align:center">Structure</th>
                                    @foreach ($siteDetails as $detailJabatan)
            ...
            @foreach ($daftarTunjangan as $it => $tunjangan)
                <tr>
                    <td>{{ $tunjangan->nama }} &nbsp; <a href="javascript:void(0)"><i
                        ...
                        @foreach ($siteDetails as $detailJabatan)
                    <td class="text-end">{{ 'Rp. ' . number_format($detailJabatan->{$tunjangan->nama}, 2, ',', '.') }}
                        </th>
            @endforeach
            </tr>
            @endforeach
            ...
            <tr>
                <td>BPJS Ketenagakerjaan ( {{ number_format($quotation->persen_bpjs_ketenagakerjaan, 2, ',', '.') }}% )
                    </th>
                    @foreach ($siteDetails as $detailJabatan)
                <td class="text-end">{{ 'Rp. ' . number_format($detailJabatan->bpjs_ketenagakerjaan, 2, ',', '.') }}
                    </th>
                    @endforeach
            <tr>
                <td>Provisi Seragam</th>
                    @foreach ($siteDetails as $detailJabatan)
                <td class="text-end">{{ 'Rp. ' . number_format($detailJabatan->personil_kaporlap_coss, 2, ',', '.') }}
                    </th>
                    @endforeach
            </tr>
            <tr>
                <td>Provisi Peralatan</th>
                    @foreach ($siteDetails as $detailJabatan)
                <td class="text-end">{{ 'Rp. ' . number_format($detailJabatan->personil_devices_coss, 2, ',', '.') }}
                    </th>
                    @endforeach
            </tr>
            @if ($quotation->kebutuhan_id == 3)
                <tr>
                    <td>Provisi Chemical</th>
                        @foreach ($siteDetails as $detailJabatan)
                    <td class="text-end">
                        {{ 'Rp. ' . number_format($detailJabatan->personil_chemical_coss, 2, ',', '.') }}
                        </th>
            @endforeach
            </tr>
            @endif
...
@endforeach
        ...
        <table class="bordered mt-4">
            <thead>
                <tr class="">
                    <td style="text-align:right" class="">Management Fee (MF) <span class="text-danger">*dari sub
                            total biaya</span> ( {{ $quotation->persentase }} % )</td>
                    <td style="text-align:right" class="" colspan="{{ count($siteDetails) }}">
                ...
                <tr class="">
                    <td style="text-align:right" class="fw-bold">PPN <span class='text-danger'>*dari management
                            fee</span> ( 12 % )</td>
                    <td style="text-align:right" class="" colspan="{{ count($siteDetails) }}">
                        {{ 'Rp. ' . number_format($quotation->ppn_coss, 2, ',', '.') }}</td>
                </tr>
                <tr class="">
                    <td style="text-align:right" class="fw-bold">PPh <span class='text-danger'>*dari management
                            fee</span> ( -2 % )</td>
                    <td style="text-align:right" class="" colspan="{{ count($siteDetails) }}">
