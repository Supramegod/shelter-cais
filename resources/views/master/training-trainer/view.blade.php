        ...
        <form class="card-body overflow-hidden" action="{{route('training-trainer.save')}}" method="POST">
          ...
          <div class="row mb-3">
            ...
            <div class="col-sm-10">
              ...
              <option value="">- Pilih Divisi -</option>  
                @foreach($listDivisi as $value)
                <option value="{{$value->id}}" @if($data->divisi_id == $value->id) selected @endif>{{$value->divisi}}</option>
                @endforeach
              </select>
              @if($errors->has('divisi_id'))
                <div class="invalid-feedback">{{$errors->first('divisi_id')}}</div>
          ...
          <div class="row mb-3">
            ...
            <div class="col-sm-10">
              <input type="text" id="trainer" name="trainer" value="{{$data->trainer}}" class="form-control @if ($errors->any()) @if($errors->has('trainer')) is-invalid @else   @endif @endif">
          ...
          <div class="card-body">
            <div class="col-12 text-center">
              <button id="btn-update" class="btn btn-primary w-100 waves-effect waves-light">
                <span class="me-1">Update Trainer</span>
...
<script>
  ...
  $('#btn-kembali').on('click',function () {
    window.location.replace("{{route('training-trainer')}}");
  });
</script>
@endsection